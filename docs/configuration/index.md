...
>>>>>>> f60317570dd00f5abd4a37969faa5e0b1662bd66
### Overlord Discovery
...
|`druid.indexer.task.enableInMemoryBitmap`| Bitmap index is supported for string type (other types don't have bitmap index support at the moment) columns in `dimensionSpec` by the flag `createBitmapIndex` (defaults to true) in immutable segments served by historicals. Optionally, if `druid.indexer.task.enableInMemoryBitmap` is true, then for string dimensions with `createBitmapIndex` flag as true in `dimensionSpec`, bitmap index will be enabled in incremental index while the segment is served by peon tasks in middle manager and queries can leverage the bitmaps to avoid a full scan to speed up querying real-time data. If either flag is false, no in-memory bitmap index will be created for the dimension.|false|
